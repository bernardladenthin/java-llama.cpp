 ...
 * {@code n_draft_accepted_total}, {@code n_draft_verif_steps_total},
 * {@code n_accepted_per_pos_total}) and a {@code slots} array.
 * </p>
 * <p>
 ...
 * by that transport split. The speculative-decoding counters keep upstream's own
 * pre-b10408 key spellings so the payload is a faithful reproduction;
 * {@code n_prompt_tokens_cached_total} is the one genuinely new key (the underlying
 * {@code n_prompt_cached} counter did not exist before b10408).
 * </p>
 *
...
public final class ServerMetrics {
    ...
    public long getDraftVerifyStepsTotal() {
        return node.path("n_draft_verif_steps_total").asLong(0L);
    }
    ...
    public List<Long> getDraftAcceptedPerPosition() {
        JsonNode perPos = node.path("n_accepted_per_pos_total");
        if (!perPos.isArray()) {
