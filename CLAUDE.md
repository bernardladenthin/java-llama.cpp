...
| ~b8854–b8887 | `common/chat.h` | `common_chat_msg_diff_to_json_oaicompat` removed; moved to `tools/server/server-chat.h`; project imports it via `#include "server-chat.h"` (adding `tools/server/server-chat.cpp` to jllama sources and `tools/server` to include dirs in CMakeLists.txt) |
